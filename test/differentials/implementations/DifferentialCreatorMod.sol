...
contract DifferentialCreatorMod {
    ...
    function create(uint256 value, bytes memory initcode) external payable returns (address deployment) {
        ...
        assembly { deployment := create(value, add(initcode, 0x20), mload(initcode)) }
        require(deployment != address(0));
    }

    function create2(bytes32 salt, uint256 value, bytes memory initcode) external payable returns (address deployment) {
        require(msg.sender == runner);
        assembly { deployment := create2(value, add(initcode, 0x20), mload(initcode), salt) }
        require(deployment != address(0));
    }
